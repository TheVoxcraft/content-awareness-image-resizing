...
public class ContentAwareImageResizing {
    ...
    public int[] leastContrastImageVPath() {
        ...
        ShortestPathsTopological imageShortestPathGraph = new ShortestPathsTopological(G, sx*sy);
        int[] path = new int[sy];
        Stack<Integer> stack = imageShortestPathGraph.pathTo(sx*sy+1);
        int i = stack.size()-2;
        stack.pop();
        while(i>0){
            path[nodeToCoordinate(stack.peek()).y] = nodeToCoordinate(stack.pop()).x;
            i--;
